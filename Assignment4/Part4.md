...
# Reading semistructured data (JSON) using pyspark and submitting the job
...
#df.createOrReplaceTempView('UNDATA2')

#df2 = spark.sql("SELECT Country, 'Mean age of women at birth of first child', 'Computer use, 16-24' \
#                   FROM UNDATA2 \
#                   WHERE ''Mean age of women at birth of first child' < 25  AND \
#                   WHERE 'Computer use, 16-24' > 10 \
#                   GROUPBY Country)")
...
`gcloud dataproc jobs submit pyspark upload_json.py     --cluster=${CLUSTER}     --region=${
REGION}`
